...
def process_pdf(pdf_path):
    ...
    try:
            ...
            for config in table_config.TableConfigEnum:
                if second_all_title[config.second_title_all]["is_continuous"]:
                    target_page, next_page = get_target_page(second_all_title[config.second_title_all]["all_titles"],
                                                             config.second_title)
                    table_start_pages, table_parent_start_pages, table_end_page = get_table(pdf, target_page, next_page,
                    ...
                    result = {
                        ...
                        '父亲开始页码': table_parent_start_pages,
                        '结束页码': table_end_page,
                        '引用全文': full_text,
        ...
        print(f'{os.path.basename(pdf_path)}出错了:{e.with_traceback(None)}')
